...
const HolidayListPage = () => {
  ...
  const canUpdateHoliday = hasPermission(user, "holiday", "update");
  const canDeleteHoliday = hasPermission(user, "holiday", "delete");
  const canCreateHoliday = hasPermission(user, "holiday", "create");
  ...
  const columns = [
    ...
    },
    {
      title: "Date",
      data: "date",
      render: (data) => `<span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-bold bg-indigo-50 text-brand-700 border border-indigo-100/50 font-mono">${formatDate(data)}</span>`
    },
    {
      title: "Day of Week",
      data: "day",
      render: (data) => `<span class="text-xs font-medium text-slate-500 bg-slate-100 border border-slate-200/50 px-2 py-0.5 rounded-md uppercase tracking-wider">${data}</span>`
    },

    ...((canUpdateHoliday || canDeleteHoliday)
      ? [
          {
            title: "Action",
            data: null,
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 tracking-tight">Holidays Master</h1>
          <p className="text-xs text-slate-500 mt-1">Configure and manage corporate annual holiday calendars for all branches.</p>
        </div>
            ...
            drawCallback: function () {
              ...
              document.querySelectorAll('.edit-btn').forEach(btn => {
                btn.onclick = (e) => {
                  ...
                  navigate(route(user,`/holidays/${id}`));
                };
              });
