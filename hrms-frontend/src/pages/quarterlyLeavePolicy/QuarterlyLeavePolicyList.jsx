...
const QuarterlyLeavePolicyList = () => {
  ...
  const handleApplyAllocation = async (id) => {
    const policy = items.find((item) => String(item._id) === String(id));
    const organizationName = policy?.organization?.name || "the selected organization";
    const confirmApply = window.confirm(
      `Are you sure you want to apply this quarterly leave allocation to all active employees in ${organizationName}?`
    );
  ...
  const columns = [
      ...
      render: (data) => {
        return `
          <div class="flex items-center gap-1.5 whitespace-nowrap">
            ...
            <button class="apply-btn p-1.5 rounded-lg bg-amber-50 text-amber-600 hover:bg-amber-600 hover:text-white border border-amber-100 hover:border-amber-600 transition-all duration-200 cursor-pointer" data-id="${data._id}" title="Apply Allocation to organization employees">
    ...
    }] : []),
    {
      title: 'Leave Type',
      data: null,
      render: (data) => `<span class="font-mono text-xs font-bold text-slate-500 bg-slate-100 border border-slate-200 px-2 py-0.5 rounded-lg whitespace-nowrap">${data?.leaveType?.code || "N/A"}</span>`,
    },
    {
      title: 'Organization',
      data: null,
      render: (data) => `<span class="font-semibold text-slate-600 whitespace-nowrap">${data?.organization?.name || "N/A"}</span>`,
    },
    {
      title: 'Year',
      data: 'year',
      render: (data) => `<span class="text-slate-600 font-medium whitespace-nowrap">${data}</span>`
    },
    {
      title: 'Quarter',
      data: 'quarter',
      render: (data) => `<span class="bg-indigo-50/60 text-brand-700 text-xs font-extrabold px-2 py-0.5 rounded border border-indigo-100/50 whitespace-nowrap">${data}</span>`
    },
    {
      title: 'Days',
      data: 'leaveDays',
      render: (data) => `<span class="font-bold text-slate-800 text-sm whitespace-nowrap">${data}</span>`
    },
    {
      title: 'Status',
      data: 'status',
    ...
    },
    {
      ...
      render: (data) => {
        ...
        return `<span class="inline-flex whitespace-nowrap items-center px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider border ${badge}">${display}</span>`;
      }
    },
    {
      title: 'Carry Forward',
      data: 'carryForward',
      render: (data) => data 
        ? `<span class="text-emerald-600 font-semibold text-xs inline-flex items-center gap-1 whitespace-nowrap"><svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg> Yes</span>` 
        : `<span class="text-slate-400 text-xs whitespace-nowrap">No</span>`,
    },
  ];
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-x-auto w-full p-6">
        <DataTable
          key={JSON.stringify(items.map((item) => [item?._id || item?.id, item?.updatedAt, item?.status, item?.isActive, item?.name, item?.code, item?.organization?._id])) || 'empty'}
          data={items}
          columns={columns}
          options={{
            paging: true,
